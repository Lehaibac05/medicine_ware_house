...
function AlertsTable({ filters }: AlertsTableProps) {
  ...
  const filteredAndSortedAlerts = useMemo(() => {
    ...
    if (filters.severity !== 'all') {
      result = result.filter(alert => alert.severity.toUpperCase() === filters.severity)
    }
    ...
    if (filters.sortBy === 'date-desc') {
      ...
      result.sort((a, b) => {
        const severityA = severityOrder[a.severity.toUpperCase() as keyof typeof severityOrder] || 0
        const severityB = severityOrder[b.severity.toUpperCase() as keyof typeof severityOrder] || 0
  ...
  const columns: ColumnsType<AlertRow> = [
    {
      ...
      dataIndex: 'alertId',
      key: 'alertId',
      render: (value: number) => <Text strong>ALT-{value.toString().padStart(4, '0')}</Text>,
      width: 120,
    },
    {
      title: 'Alert Type',
    ...
    },
    {
      ...
      render: (_: unknown, record: AlertRow) => (
        <Space>
          <Button 
            size="small"
            onClick={() => messageApi.info(record.message)}
          >
            View
          </Button>
