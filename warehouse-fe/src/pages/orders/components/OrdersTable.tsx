...
function OrdersTable({ filters, onSearch }: OrdersTableProps) {
    ...
    try {
      ...
      const mapped: OrderRow[] = data.map((order: Order) => {
        const itemsSummary = order.items
          .map(
            (item) => `${item.medicineName || "Medicine"} (${item.quantity})`,
          )
          .join(", ");
  ...
  const filteredAndSortedOrders = useMemo(() => {
    ...
    if (filters.status !== "all") {
      result = result.filter(
        (order) => order.status.toUpperCase() === filters.status,
      );
    }
  ...
  const columns: ColumnsType<OrderRow> = [
    {
      ...
      key: "orderId",
      render: (value: number) => (
        <Text strong>ORD-{value.toString().padStart(4, "0")}</Text>
      ),
      width: 120,
    },
    {
    ...
    },
    {
      ...
      render: (_: unknown, record: OrderRow) => (
        <Space>
          <Button size="small" onClick={() => showOrderDetails(record)}>
            View
          </Button>
